<div>
                            ...
                            @foreach ($schedules as $schedule)
                                <tr class="hover:bg-gray-100 text-center text-xs text-black cursor-pointer">
                                    ...
                                    <td class="flex justify-around">
                                        ...
                                        <button wire:click="viewDeleteShipmentSchedule('{{ $schedule->id }}')" class="text-white bg-[#C30010] rounded-lg p-2 my-2 cursor-pointer hover:opacity-90">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4">
    ...
    <x-modal 
        title="Hapus Data Jadwal Pengiriman"
        name="view-delete-shipment-schedule">
        <x-slot:icon>
            <x-icons.round-warning class="size-5 text-red-600" />
        </x-slot:icon>

        @if ($selectedSchedule)
            <h2 class="font-semibold text-md">Apakah anda yakin ingin menghapus data jadwal pengiriman ini?</h2>
            <div class="flex flex-col text-gray-800 text-sm my-2 space-y-1">
                <div class="flex">
                    <span class="w-28">Klien</span>
                    <span>: {{ $selectedSchedule->client }}</span>
                </div>
                <div class="flex">
                    <span class="w-28">Harga Pengiriman</span>
                    <span>: {{ $selectedSchedule->delivery_price }}</span>
                </div>
                <div class="flex">
                    <span class="w-28">Tanggal Pengiriman</span>
                    <span>: {{ $selectedSchedule->departure_date }}</span>
                </div>
            </div>

            <form wire:submit.prevent="deleteSchedule" class="flex justify-end">
