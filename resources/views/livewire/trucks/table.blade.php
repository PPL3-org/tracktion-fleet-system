<div>
                            ...
                            <tr 
                                ...
                                <td class="flex justify-around">
                                    ...
                                    <button 
                                        ...
                                        class="text-white bg-[#FFB700] rounded-lg p-2 my-2 cursor-pointer hover:brightness-110 transition">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                        </svg>
                                    </button>
                                    <button 
                                        wire:click.prevent="deleteTruck('{{ $truck->id }}')"
                                        onclick="return confirm('Anda yakin ingin menghapus data truk ini?')"
    ...
    <x-modal title="Ubah data truk" name="edit-truck-view" focusable>
        <form wire:submit.prevent="updateTruck" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" wire:model="selectedTruck.plate_number" 
                    class="w-full mt-1 p-2 border rounded">
                @error('selectedTruck.plate_number') <span class="text-sm text-red-500">{{ $message }}</span> @enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Model Truk</label>
                <input type="text" wire:model.defer="selectedTruck.model" 
                    class="w-full mt-1 p-2 border rounded">
                @error('selectedTruck.model') <span class="text-sm text-red-500">{{ $message }}</span> @enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">Total Jarak Tempuh (Km)</label>
                <input type="number" wire:model.defer="selectedTruck.total_distance" 
                    class="w-full mt-1 p-2 border rounded">
                @error('selectedTruck.total_distance') <span class="text-sm text-red-500">{{ $message }}</span> @enderror
            </div>
